        ...
        <div className="relative w-auto min-h-[50vh] sm:min-h-[60vh] md:min-h-[70vh] lg:min-h-[80vh] flex items-center justify-center overflow-hidden hero">
            ...
            <div className="w-full h-full max-w-7xl mx-auto">
                {slides.map((slide, index) => (
                    ...
                    >
                        {/* Text Content */}
                        <div className="w-full md:w-1/2 p-8 md:pl-12 lg:pl-20 flex flex-col justify-center gap-4 sm:gap-6 md:gap-8 lg:gap-10 text-center md:text-left lg:ml-16">
                            <h2 className="text-3xl sm:text-4xl md:text-4xl xl:text-5xl font-semibold text-neutral-900">NEW ARRIVALS ONLY</h2>
                            <div>
                                <div className="flex items-center justify-center md:justify-start gap-2 sm:gap-4">
                                    <p className="text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-medium">New</p>
            ...
            <div className="absolute bottom-4 left-0 right-0 flex justify-center space-x-2 z-10">
                {slides.map((_, index) => (
                    <button
                        ...
                        className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full bg-white ${index === currentIndex ? 'opacity-100' : 'opacity-50'} hover:opacity-100 transition-opacity duration-300`}
            ...
            {/* Controls */}
            <button
                className="absolute top-1/2 left-2 sm:left-4 md:left-6 transform -translate-y-1/2 bg-secondary hover:bg-secondary-content hover:text-secondary p-1 sm:p-2 md:p-3 rounded-full text-white transition-colors duration-300 z-10"
                type="button"
                onClick={prevSlide}
            >
            ...
            <button
                className="absolute top-1/2 right-2 sm:right-4 md:right-6 transform -translate-y-1/2 bg-secondary hover:bg-secondary-content hover:text-secondary p-1 sm:p-2 md:p-3 rounded-full text-white transition-colors duration-300 z-10"
                type="button"
                onClick={nextSlide}
            >
